...
| **`/etc/hosts`** | `127.0.0.1 renis.local` |
...
Each service is on its own `*.renis.local` subdomain. Add this line to
`/etc/hosts` (`%SystemRoot%\System32\drivers\etc\hosts` on Windows):

```
127.0.0.1 renis.local management.renis.local widget.renis.local auth.renis.local data.renis.local
```

### 1.2 Dev TLS certificate

Traefik terminates TLS with a self-signed wildcard cert (`renis.local` and
`*.renis.local`) generated by:

```bash
sh infrastructure/traefik/generate-cert.sh
```

The cert lives in `infrastructure/traefik/certs/renis.local.{crt,key}`
...
1. Open **https://management.renis.local/**
...
3. Optional health check: **https://management.renis.local/api/health**

**Login tips:**

- Use the **management** login at **management.renis.local**, not the Keycloak Admin Console (`admin` / `admin` at **auth.renis.local/admin** is only for realm `master`).
- Browser redirects to Keycloak realm **`renis`** at **auth.renis.local/realms/renis/** in the URL.

**Widget verify:** **https://renis.local/verify** (TYPO3 hosts the page; the
widget bundle is at **widget.renis.local/renis-verify.iife.js**). After seed,
...
This runs TYPO3 setup (if the `typo3` database is empty), activates `renis_verify`, and creates **https://renis.local/verify**.

### Option B — Install Tool (browser)

1. `docker compose up -d typo3`
2. Open **https://renis.local/typo3/install.php** (not `/typo3` on first install).
...
**https://renis.local/typo3/login?loginProvider=1616569531**
...
Included in `setup-typo3.sh` / container startup. If **https://renis.local/verify** is missing (404 or empty page):
...
Then open **https://renis.local/verify** (widget script from **/widget/**, API from **/api/**).
...
1. https://auth.renis.local/admin — realm **`master`**, user `admin` / password from `.env`
...
Everything is served through Traefik on `*.renis.local`. The browser will
warn about the self-signed certificate the first time.

| URL | Service |
|-----|---------|
| https://renis.local | TYPO3 frontend |
| https://renis.local/verify | Public verify page (after §9, TYPO3-rendered) |
| https://management.renis.local | Management UI |
| https://management.renis.local/api/health | Management API health check |
| https://management.renis.local/api/verify/{code} | Public verification API |
| https://auth.renis.local | Keycloak account console |
| https://auth.renis.local/admin | Keycloak admin (`master` realm) |
| https://widget.renis.local | Widget demo + `renis-verify.iife.js` |
| https://data.renis.local | MinIO S3 endpoint / console |
...
| API verify | `curl -k https://management.renis.local/api/verify/00000000-0000-4000-a000-000000000001` |
| Widget | https://renis.local/verify |
...
KEYCLOAK_ISSUER=https://auth.renis.local/realms/renis
KEYCLOAK_INTERNAL_ISSUER=https://auth.renis.local/realms/renis
